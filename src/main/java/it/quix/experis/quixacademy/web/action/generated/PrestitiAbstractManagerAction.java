package it.quix.experis.quixacademy.web.action.generated;
...
import it.quix.experis.quixacademy.core.model.*;
...
import it.quix.experis.quixacademy.core.manager.QuixacademyException;
// import it.quix.experis.quixacademy.core.handler.PrestitiHandler;
import it.quix.experis.quixacademy.web.action.QuixacademyManagerAction;
 ...
 */
public class PrestitiAbstractManagerAction extends QuixacademyManagerAction {
     ...
     * 
     * @throws QuixacademyException if an error occurs
     */
    public String list() throws QuixacademyException {
        try {
            // Validate the search model
            getQuixacademyManager().validatePrestitiSearch(prestitiSearch);
            // Perform count of record that satisfy search filters
            long total = getQuixacademyManager().countPrestiti(prestitiSearch);
            // If there are results ...
            List<Prestiti> prestitiList = null;
            if (total > 0) {
                // Search the results to display
                do {
                    prestitiList = getQuixacademyManager().getPrestitiList(prestitiSearch);
    ...
    public String exportXls() throws Exception {
        ...
        List<Prestiti> prestitiList = getQuixacademyManager().getPrestitiList(prestitiSearch);
        long countPrestiti = getQuixacademyManager().countPrestiti(prestitiSearch);
    ...
    public String edit() {
        ...
        try {
            prestiti = getQuixacademyManager().getPrestiti(prestiti.getBeneficiario_username(), prestiti.getOggettoPrestato_id());
    ...
    public String save() {
        ...
        try {
            getQuixacademyManager().savePrestiti(prestiti);
    ...
    public String delete() {
        try {
            getQuixacademyManager().deletePrestiti(prestiti.getBeneficiario_username(), prestiti.getOggettoPrestato_id());
            return manageOkMessage();
        } catch (Exception e) {
