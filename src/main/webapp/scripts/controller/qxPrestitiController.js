...
quixacademyApp.controller('qxPrestitiController', ['$scope', 'qxQuixacademyHttpService', '$modal', 'labelService', 'SweetAlert', 'quixacademyConfig', function ($scope, qxQuixacademyHttpService, $modal, labelService, SweetAlert, quixacademyConfig){
	...
	$scope.list = function () {
		qxQuixacademyHttpService.getPrestitiList($scope.scopeController, $scope.forms.prestitiListForm);
	}
	
	$scope.edit = function(row){
		$scope.scopeController.selectedRow = row;
		qxQuixacademyHttpService.editPrestiti($scope.scopeController);
	}
	
	$scope.exportXLS = function() {
		document.location.href =  quixacademyConfig.baseUrl + '/prestiti.action?task=exportXls&reset=true&' + quixParamSerializer($scope.scopeController.search, 'prestitiSearch.');
	}
	...
	$scope.delete = function(row){
		 SweetAlert.swal({
			...
			if(isConfirm){
				$scope.scopeController.selectedRow = row;
				qxQuixacademyHttpService.deletePrestiti($scope.scopeController, labelService);
			}
		});
	}
	...
	$scope.save = function(row){
		qxQuixacademyHttpService.savePrestiti($scope.scopeController, $scope.forms.prestitiEditForm);
	}
...
quixacademyApp.controller('qxPrestitiPopupController', ['$scope', 'qxQuixacademyHttpService', '$modal', '$modalInstance', '$controller', function ($scope, qxQuixacademyHttpService, $modal, $modalInstance, $controller) {
