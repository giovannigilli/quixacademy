...
quixacademyApp.controller('qxOggettiController', ['$scope', 'qxQuixacademyHttpService', '$modal', 'labelService', 'SweetAlert', 'quixacademyConfig', function ($scope, qxQuixacademyHttpService, $modal, labelService, SweetAlert, quixacademyConfig){
	...
	$scope.list = function () {
		qxQuixacademyHttpService.getOggettiList($scope.scopeController, $scope.forms.oggettiListForm);
	}
	
	$scope.edit = function(row){
		$scope.scopeController.selectedRow = row;
		qxQuixacademyHttpService.editOggetti($scope.scopeController);
	}
	
	$scope.exportXLS = function() {
		document.location.href =  quixacademyConfig.baseUrl + '/oggetti.action?task=exportXls&reset=true&' + quixParamSerializer($scope.scopeController.search, 'oggettiSearch.');
	}
	...
	$scope.delete = function(row){
		 SweetAlert.swal({
			...
			if(isConfirm){
				$scope.scopeController.selectedRow = row;
				qxQuixacademyHttpService.deleteOggetti($scope.scopeController, labelService);
			}
		});
	}
	...
	$scope.save = function(row){
		qxQuixacademyHttpService.saveOggetti($scope.scopeController, $scope.forms.oggettiEditForm);
	}
...
quixacademyApp.controller('qxOggettiPopupController', ['$scope', 'qxQuixacademyHttpService', '$modal', '$modalInstance', '$controller', function ($scope, qxQuixacademyHttpService, $modal, $modalInstance, $controller) {
